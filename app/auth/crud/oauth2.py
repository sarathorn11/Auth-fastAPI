...
def save_otp(db:Session,request:schema.CreateOTP,session_id:int,otp_code:str,status):
...
def find_otp_lifetime(db:Session,request:schema.VerifyOTP):
    return db.query(models.Otps).filter(models.Otps.recipient_id == request.recipient_id ,
     models.Otps.session_id == request.session_id , models.Otps.created_on >= datetime.now(pytz.timezone('Asia/Phnom_Penh'))).first()
def update_otp_failed_count(db:Session,request:schema.OTPList):
...
def save_block_otp(db:Session,request:schema.OTPList):
...
def disable_otp_code(db:Session,request:schema.OTPList):
    otp_code = db.query(models.Otps).filter(models.Otps.status == "1", models.Otps.recipient_id==request.recipient_id)
